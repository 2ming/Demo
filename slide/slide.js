...
function SlideLoad(opts){
	...
	this.scrollY = opts.scrollY ? opts.scrollY : true;
...
SlideLoad.prototype = {
	init : function(){
		...
		this.innerHt = this.outer.offsetHeight; //inner高度
		this.slideHt = this.wrap.offsetHeight; //容器高度
		this.maxoffsetY = Math.abs(this.innerHt-this.slideHt); //最大偏移值
		//this.aaaa = 0;
		...
		this._translate(this.outerX,this.outerY)
	...
	renderDOM : function(loadList){
		...
		if(this.loadType == 'down'){
			...
			this.outer.insertBefore(this.inner,this.outer.children[1]);
		}	
	...
	bindDOM : function(){
		...
		var startHandler = function(evt){
			...
			self.innerHt = self.outer.offsetHeight; //inner高度
			self.slideHt = self.wrap.offsetHeight; //容器高度
			self.maxoffsetY = Math.abs(self.innerHt-self.slideHt);
		...
		var moveHandler = function(evt){
			...
			evt.preventDefault();
			self.loadType = self.offsetY>0?'up':'down';

			//计算手指的偏移量
			self.offsetY = evt.targetTouches[0].pageY - self.startY;
			//self.translate = parseInt(self.translate)+self.offsetY;

			self.outerY = parseInt(self.scrollTop)+self.offsetY;
			self._translate(self.outerX,self.outerY)

			if(self.maxoffsetY < Math.abs(self.outerY)-50){
		...
		var endHandler = function(evt){
			...
			//拉动 加载内容容器下面
			if((self.loadUp && self.outerY >= 0 && self.offsetY >0) || (self.loadDown && self.maxoffsetY <= Math.abs(self.outerY))){
			...
			//滑动到最上面或最下面
			if(self.outerY >0 || self.maxoffsetY < Math.abs(self.outerY)){
				
				if(self.offsetY>0 || self.slideHt>self.innerHt){
					self.outerY = 0;
				}else{
					self.outerY = self.slideHt-self.innerHt;
				}
