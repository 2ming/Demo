...
function Slider(opts){
	...
	this.refresh = opts.refresh?opts.refresh:false;//刷新内容
	if(opts.ajaxLoadFun) this.ajaxLoadFun = opts.ajaxLoadFun;//加载函数
	if(opts.refreshFun) this.refreshFun = opts.refreshFun;//加载汽车函数
...
Slider.prototype = {
	init : function(){
		...
		var maxWindow = window.innerWidth>maxWidth?maxWidth:window.innerWidth; //手机屏幕的宽度;
...
//			console.log(maxRowWidth)
	...
	bindDOM : function(){
		...
		var startHandler = function(evt){
			...
			if (!self.outer ||!evt.touches.length) return;
			...
			self.outer.addEventListener('touchmove', moveHandler,false);
			self.outer.addEventListener('touchend', endHandler,false);		
		};

		//手指移动的处理事件
		var moveHandler = function(evt){
			var endY = evt.touches[0].pageY;
			var endX = evt.touches[0].pageX;
			...
			//获取滑动的坐标
			self.targetYX = self.sliderdir?evt.targetTouches[0].pageY:evt.targetTouches[0].pageX;
		...
		var endHandler = function(evt){
			...
			self.outer.removeEventListener('touchmove', moveHandler,false);
			self.outer.removeEventListener('touchend', endHandler,false);
			
		};

		//绑定事件
		//ajax未开启 数量不足 不绑定滑动事件
		if(this.load || this.children.length>this.col*this.row){
